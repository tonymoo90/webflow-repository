...
fetch('https://api.airtable.com/v0/appzvtjAGocJzOExM/Member.Bets?filterByFormula=NOT({Member ID}!=' + JSON.stringify(JSON.parse(localStorage.getItem('memberstack')).information.id) + ')', {
	...
	.then(data => {
			...
			$('.popup-close-btn_1').click(function() {
				togglePopup(false);
				$('.confirm_button_yes, .confirm_button_no').unbind()
			})
			$('.popup-close-btn_2').click(function() {
				togglePopup(true);
				$('.confirm_button_yes, .confirm_button_no').unbind()
			})
							...
							setTimeout(() => {
								fetch('https://api.airtable.com/v0/appzvtjAGocJzOExM/Member.Bets?filterByFormula=NOT({Member ID}!=' + JSON.stringify(JSON.parse(localStorage.getItem('memberstack')).information.id) + ')', {
									...
									.then(json => {
										...
										setTimeout(() => {
											document.getElementById("txtNumber").value = newWalletBalance
										}, 2000);
									})
							}, 5000);
						}
